...
def load_wikitext103(path: str = '.', seq_length: int = 32) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    train_iter = WikiText103(root=path, split='train')
    tokenizer, vocab = get_tokenizer_vocab(train_iter)

    train_iter, val_iter, test_iter = WikiText103(root=path)
