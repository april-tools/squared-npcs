...
def plot_mixture_ellipses(mixture: TensorizedPC, ax: plt.Axes):
    ...
    if isinstance(mixture, MonotonicPC):
        ...
        # mix_weights = np.array([mix_weights[0] ** 2, mix_weights[1] ** 2, 2.0 * mix_weights[0] * mix_weights[1]])
        # # Products of Gaussian pdfs
        # new_covs = np.array([
        #     covs[:, 0] / 2.0,
        #     covs[:, 1] / 2.0,
        #     (covs[:, 0] * covs[:, 1]) / (covs[:, 0] + covs[:, 1])
        # ]).T  # New covariances
        # new_mus = np.array([
        #     mus[:, 0],
        #     mus[:, 1],
        #     new_covs[:, 2] * (mus[:, 0] / covs[:, 0] + mus[:, 1] / covs[:, 1])
        # ]).T  # New means
        # num_components = 3
        # mus = new_mus
        # covs = new_covs
...
if __name__ == '__main__':
    ...
    for idx, (p, pdf, m, nc) in enumerate(data_pdfs):
        ...
        plt.savefig(os.path.join('figures', 'gaussian-ring', f'pdfs-ellipses-{idx}.png'), dpi=1200)
