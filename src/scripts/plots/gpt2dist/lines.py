...
if __name__ == '__main__':
    ...
    markers = ['o', 'D']
    num_points = 6
    for k, model_name in enumerate(['MonotonicPC', 'BornPC']):
        ...
        for j, hparam_df in group_model_df:
            ...
            if len(np.unique(ms)) < num_points or len(np.unique(ps)) < num_points:
                continue
            #hparam_df.to_csv(f'{model_name}-{j}.csv')
        ...
        if args.median:
            label = f'{format_model(model_name)}' if should_label else None
            median_metrics = sorted(list(map(lambda x: (x[0], np.median(x[1])), metrics.items())), key=lambda x: x[0])
            bot_metrics = sorted(list(map(lambda x: (x[0], np.quantile(x[1], q=0.1)), metrics.items())), key=lambda x: x[0])
            top_metrics = sorted(list(map(lambda x: (x[0], np.quantile(x[1], q=0.8)), metrics.items())), key=lambda x: x[0])
